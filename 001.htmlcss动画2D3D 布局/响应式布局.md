...
* 媒体查询：档位划分；**市场上默认**的划分；三个节点、四个档位
  ...
  * 768<= w <992  **小屏设备（sm : small）s**（平板）
  * 992<= w <1200  **中等屏幕（md: medium）m**（桌面显示器）
...
* 为什么要设置版心？
  * 所有的子元素都是归于版心下，
  * 不同的版心宽度，意味着子元素要以不同的布局排版满足**用户浏览友好**的需求；
...
- 多个使用类前缀会单独生效？
  - 若当前屏幕属于某个档位，直接该档位类前缀生效；
  - 其他不生效的类名对于生效的类名来说，空类型；
